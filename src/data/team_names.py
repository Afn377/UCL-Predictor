...
def club_id(canonical_name: str) -> str:
    # uuid5 stays stable across provider renames
    return str(uuid5(NAMESPACE_URL, "ucl-forecast:club:" + canonical_name))
...
def load_team_name_mapping(path: Path) -> dict[str, str]:
    ...
    # one source spelling can't map to two clubs
    ...
    return dict(
        zip(mapping_df["source_name"], mapping_df["canonical_name"], strict=False)
    )


def normalize_team_name(name: object, mapping: dict[str, str]) -> object:
    # keep NaNs so joins and coverage still see them
    if pd.isna(name):
        return name
...
def find_unmapped_ucl_style_names(matches: pd.DataFrame) -> tuple[str, ...]:
    # "(XXX)" suffix hints an unmapped OpenFootball name
    ...
    unmapped = raw_names[
        ...
        & raw_names.astype(str).str.contains(r"\([A-Z]{3}\)$")
    ]
