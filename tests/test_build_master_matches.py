...
def test_build_master_matches_drops_blank_rows_and_sorts_by_date(
    tmp_path: Path,
) -> None:
...
def test_build_master_matches_preserves_raw_names_and_applies_mapping(
    tmp_path: Path,
) -> None:
    raw_dir = make_raw_tree(tmp_path)
    ...
    matches, report = build_master_matches(
        raw_dir=raw_dir, team_mapping_path=mapping_path
    )
    ucl_row = matches[matches["competition"] == "Champions League"].iloc[0]
